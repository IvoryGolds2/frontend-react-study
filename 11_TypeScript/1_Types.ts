...
// (참고) https://hwani.dev/js-primitive-reference-types/
let person2: { name: string, age: number } = { name: 'Ivory', age: 29 };
console.log(person2);
// 이 변수에는 객체만 들어 올 수 있는데 name은 string, age는 number 타입만 가능
let person3: { name: string, age: number, job?: string } = { name: 'Ivory', age: 29 };
// ? 사용 시 jon 속성은 옵셔널
...
// 타입은 명시적으로 지정
 ...
 let ivory: {job: string, studyHours: number} = {job: 'cellist', studyHours: 5}
...
let favoriteSong: {singer: string, song: string } = {singer: '선우정아', song: '구애'}
...
// 함수의 리턴 타입으로 자주 사용됨
function storeValueInDb(objectStore: object): void {
  // 데이터베이스에 값을 저장
}
// 이 함수는 객체를 받아서 데이터베이스에 저장하지만 아무것도 반환하지 않기 때문에 리턴 타입을 void로 지정(void는 리턴하는게 없다는 의미)

// 또는 화살표 함수 사용 시 
const storeValueInDb2 = (objectStore: object): void => {
  // 데이터베이스에 값을 저장
}
...
let userInfo1: (string | number | undefined | boolean)[] = [userName, userAge, married];
