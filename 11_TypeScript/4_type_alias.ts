...
// type이 길어지거나 타입을 재사용 하고 싶다면 변수처럼 만들어 사용 가능

// 영어 대문자로 시작하는게 관례, 가독성을 위해 뒤에 Type를 붙여주기도 함
// (주의) type 선언도 변수 선언과 마찬가지로 중복 선언 불가
...
// friend.name = 'Ivory'; // readonly로 읽기 전용 속성임
console.log(friend); // 실제로 JS에서는 값이 바뀌는데 TS에서는 에러를 발생시켜줌
// (타입스크립트 에러는 에디터&터미널에서만 발생하고 실제 실행과는 상관 없음)
...
type Alias3 = Alias2 & Adult

// Literal Type로 더 엄격한 타입 지정하기
// 변수에 뭐가 들어올 지 더 엄격하게 관리 가능
// 자동 완성 기능이 동작함
// 특정 문자만 들어 올 수 있도록 지정
let hisName: 'kim' | 'gold';
hisName = 'kim'; // 자동완성 기능 동작
// hisName = 'ivory' // 에러 발생
...
function RCP(rcp:'가위' | '바위' | '보'): ('가위' | '바위' | '보')[] {
  return ['가위','바위'];
}
