...
function App() {
  ...
  // 추가, 수장, 삭제 각 함수에 넣어도 되지만, useEffect()를 활용하면 한번에 처리 가능
  useEffect(() => {
    localStorage.setItem('todos', JSON.stringify(todos));
  }, [todos]);

  // 새 객체를 만들 때마다 id값에 1씩 더해주어야 하는데 useRef()를 사용하여 변수 생성
  // Quiz: state가 아닌 이유? id값은 렌더링되는 정보가 아니기 때문 (화면에 보이지도 않고, 이 값이 바뀐다고 해서 컴포넌트가 재렌더링 될 필요도 없기 떄문)
  // 단순히 새로운 항목을 만들 때 참조되는 값임
  const nextId = useRef(4); // Ref 예제 참조!!
  ...
  const handleInsert = useCallback((text) => {
    ...
    // todos에 넣는 방법1 - 이전까지 쓰던 방법
    // const copyTodos = [...todos];
    // copyTodos.push(todo); // 혹은 const copyTodos = [...todos, todo];
    // setTodos(copyTodos); // 새로운 배열을 만들어 넣어줌

    // todos에 넣는 방법2 - 배열의 내장 함수 이용
    // 함수형 업데이트를 쓸 경우 의존성을 제거해 줄 수 있음
    setTodos(todos => todos.concat(todo)); // 콘캣을 쓰면 새로운 배열을 반환
  ...
  const handleRemove = useCallback((id) =>{
    ...
    // 방법2
    // 불변성을 지키면서 배열의 요소를 제거해야 할 때 filter()를 활용
