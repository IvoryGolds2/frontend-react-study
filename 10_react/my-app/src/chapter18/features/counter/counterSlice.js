...
// 인자값으로 name, initalState, reducers 속성을 갖는 객체를 넣음
export const counterSlice = createSlice({
  name: 'counter', // state 이름(추가로 action 이름을 만드는데도 쓰임)
  initialState, // state의 초기값 위에서 객체 형태로 정의! (변수 이름과 속성 이름 같으면 생략 가능(ES6차))
  // state를 변경하는 함수(reducer)들을 정의하고 관련 action을 생성
  // 현재 state와 action 객체를 파라미터로 받아오고 필요한 경우 상태를 업데이트하고 새 상태를 반환한느 함수
  reducers: {
